    ...
    def __init__(self, message=None, opcode=None, version=None,
        ...
        if masking_key:
            ...
            if len(masking_key) != 4:
                raise ProtocolError('WEBSOCKET masking key must be 4 bytes '
                                    'long')
...
into a server frame (unmasked).'''
        ...
        if frame.masking_key is None:
            ...
            if d:
                frame.msg.extend(d)
            if mask_length:
                frame.masking_key, data = (data[:mask_length],
                                           data[mask_length:])
            else:
                frame.masking_key = b''
